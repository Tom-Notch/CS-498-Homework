...
R1, R2, P1, P2, Q, validPixROI1, validPixROI2 = cv2.stereoRectify(
    K1, None, K2, None, (w // 4, h // 4), R, t, 1, newImageSize=(0, 0)
)
...
left_img = cv2.remap(
    img1, left_map[0], left_map[1], cv2.INTER_LANCZOS4, cv2.BORDER_CONSTANT, 0
)
right_img = cv2.remap(
    img2, right_map[0], right_map[1], cv2.INTER_LANCZOS4, cv2.BORDER_CONSTANT, 0
)
plt.figure()
plt.imshow(
    cv2.cvtColor(np.concatenate((left_img, right_img), axis=1), cv2.COLOR_BGR2RGB)
)
...
def stereo_matching_ssd(left_im, right_im, max_disp=128, block_size=7):
    ...
    for i in range(bound, H - bound - 1):
        for j in range(bound, W - bound - 1):
            ssd_min = float("inf")
            for k in range(max(i - max_disp, bound), min(i + max_disp, W - bound - 1)):
                left_block = left_im[
                    (i - bound) : (i + bound + 1), (j - bound) : (j + bound + 1)
                ]
                right_block = right_im[
                    (i - bound) : (i + bound + 1), (k - bound) : (k + bound + 1)
                ]
                current_ssd = np.sum(np.square(left_block - right_block))
                if current_ssd < ssd_min:
                    ssd_min = current_ssd
                    disp_map[i][j] = abs(j - k)

    disp_map = np.clip(disp_map, 0, max_disp)
    # --------------------------- End your code here   ---------------------------------------------
    return disp_map
